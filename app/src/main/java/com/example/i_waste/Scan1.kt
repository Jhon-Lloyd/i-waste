...
import com.example.i_waste.databinding.ActivityScan1Binding
import com.example.i_waste.ml.Iwastemodel
import org.tensorflow.lite.support.image.TensorImage
import java.io.IOException

class Scan1 : AppCompatActivity() {
    private lateinit var binding: ActivityScan1Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityScan1Binding.inflate(layoutInflater)
        setContentView(binding.root)

        imageView = binding.imageView
        button = binding.picbtn
    ...
    private fun outputGenerator(bitmap: Bitmap) {
        ...
        val iwastemodel = Iwastemodel.newInstance(this)
        ...
        val outputs = iwastemodel.process(tfimage)
            .probabilityAsCategoryList.apply {
                sortByDescending { it.score }
            }

        //getting the result having high probability
        val highProbabilityOutputs = outputs[0]

        //setting output text
        result.text = highProbabilityOutputs.label
    ...
    private fun downloadImage(mBitmap: Bitmap): Uri? {
        val contentValues = ContentValues().apply {
            put(MediaStore.Images.Media.DISPLAY_NAME,
                "Waste_Images" + System.currentTimeMillis() / 1000)
